$(function () {
    ...
    function initDistrictSchool() {
        $.ajax({
            url: window.config.api + '/system/getDistrictWithSchool?additional=1',
            method: "GET",
            async: false,
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (type == 'gy' && !response.data.gyKindergarten) {
                    toastr.warning("当前时间无法进行公办幼儿园补录");
                    $("#gySchool").empty();
                }
            }
        })
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (type == 'add') {
                    ...
                    swal(
                        "补录须知",
                        "公办幼儿园补录和公益幼儿园只能选一个，如果您选择了公办补录幼儿园就不能选择公益幼儿园，如果您选择了公益幼儿园就不能选择公办补录幼儿园。"
                        , {
                            button: "确定",
                        }).then(function () {
                        if (!response.data.gyKindergarten) {
                            toastr.warning("当前时间不在公办幼儿园补录时间内");
                            setView();
                        }
                    });
